...
-- ─── ROW LEVEL SECURITY ───────────────────────────────────────────────────────
-- The frontend uses the anon key for all public reads and public writes
-- (comments, reactions). The admin panel uses the service_role key
-- which bypasses RLS entirely — so admin policies are not needed here.
...
CREATE POLICY "public_read_promises"       ON promises      FOR SELECT USING (true);
CREATE POLICY "public_read_contradictions" ON contradictions FOR SELECT USING (true);
CREATE POLICY "public_read_comments"       ON comments      FOR SELECT USING (flagged = false);
CREATE POLICY "public_read_reactions"      ON reactions     FOR SELECT USING (true);

-- Public write (comments + reactions only — everything else goes through admin)
...
CREATE POLICY "public_update_reactions" ON reactions FOR UPDATE USING (true);
...
(
  ...
  'In a press conference at Mar-a-Lago, President Trump declined to rule out military or economic coercion to acquire Greenland, stating it was "an absolute necessity" for US national security. Denmark, a NATO member, called the statement "unacceptable." The remarks sent shockwaves through European capitals.',
...
(
  ...
  'During a speech in Washington, Trump stated that NATO was "essentially useless" and that European nations had "never once helped the United States when we needed it." He questioned whether the US should remain in the alliance at all.',
...
(
  ...
  'The US military conducted targeted airstrikes on three Iranian nuclear facilities. Trump appeared on Truth Social minutes later declaring total victory. No Iranian response had yet occurred at time of posting.',
...
(
  ...
  'The White House sent formal requests to NATO member states asking for naval and air support in the Persian Gulf, citing Article 5 collective defense provisions — the same provisions Trump had weeks earlier called meaningless.',
...
(
  ...
  'The tariffs cover all Canadian imports including auto parts, lumber, and dairy. Canadian PM announced retaliatory tariffs immediately. Markets dropped 2.3% on the news.',
...
SELECT
  ...
  'Called NATO useless and said Europe never helps the US — then formally invoked Article 5 to request European military assistance weeks later.'
WHERE
...
SELECT id, 'absurd',       floor(random() * 200 + 50)::int  FROM events UNION ALL
SELECT id, 'outrageous',   floor(random() * 300 + 100)::int FROM events UNION ALL
SELECT id, 'unbelievable', floor(random() * 150 + 30)::int  FROM events UNION ALL
