...
export default function Admin() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [ev, pr] = await Promise.all([adminApi.getEvents(), adminApi.getPromises()]);
      setEvents(ev);
      setPromises(pr);
    } catch (e) {
  ...
  const saveEvent = async () => {
    try {
      if (editingEvent) {
        await adminApi.updateEvent(editingEvent, eventForm);
        flash('✓ Event updated');
      } else {
  ...
  const savePromise = async () => {
    try {
      if (editingPromise) {
        await adminApi.updatePromise(editingPromise, promiseForm);
        flash('✓ Promise updated');
      } else {
        await adminApi.createPromise(promiseForm);
        flash('✓ Promise created');
      }
      setPromiseForm(BLANK_PROMISE);
  ...
  if (!authed) return (
    <div style={{ maxWidth: 400, margin: '8rem auto', padding: '0 1.5rem' }}>
      <div style={{
        ...
        borderTop: '3px solid var(--neon-amber)', borderRadius: 8, padding: '2rem',
      }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', letterSpacing: '0.15em', color: 'var(--neon-amber)', marginBottom: '0.5rem' }}>
          ◈ RESTRICTED ACCESS
        </div>
        <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: '1.8rem', letterSpacing: '0.05em', marginBottom: '1.5rem' }}>
          ADMIN PANEL
        </h2>
        ...
        {authError && (
          <div style={{ color: 'var(--neon-red)', fontFamily: 'var(--font-mono)', fontSize: '0.75rem', marginBottom: '1rem' }}>
            {authError}
          </div>
        )}
        <button onClick={handleAuth} style={{
          width: '100%', fontFamily: 'var(--font-display)', fontWeight: 700,
          fontSize: '1rem', letterSpacing: '0.1em', padding: '0.75rem', borderRadius: 6,
          border: '1px solid var(--neon-amber)', background: 'var(--neon-amber-dim)',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        {msg && (
          <div style={{
            ...
            color: msg.startsWith('✗') ? 'var(--neon-red)' : 'var(--neon-green)',
            padding: '0.4rem 0.8rem',
            border: '1px solid ' + (msg.startsWith('✗') ? 'var(--neon-red)' : 'var(--neon-green)'),
            borderRadius: 4,
          }}>
            {msg}
          </div>
      ...
      <div style={{ display: 'flex', gap: '0.25rem', marginBottom: '2rem' }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            padding: '0.5rem 1.5rem', borderRadius: 4, cursor: 'pointer',
      ...
      {tab === 'events' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '2rem', alignItems: 'start' }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '1.5rem' }}>
            ...
            <Field label="TITLE">
              <input value={eventForm.title} onChange={e => setEventForm(p => ({ ...p, title: e.target.value }))} style={inputStyle} />
            </Field>
            <Field label="SUMMARY (shown on card)">
              <textarea value={eventForm.summary} onChange={e => setEventForm(p => ({ ...p, summary: e.target.value }))} rows={2} style={{ ...inputStyle, resize: 'vertical' }} />
            </Field>
            <Field label="BODY (full article text)">
              <textarea value={eventForm.body} onChange={e => setEventForm(p => ({ ...p, body: e.target.value }))} rows={5} style={{ ...inputStyle, resize: 'vertical' }} />
            </Field>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              ...
              <Field label="DATE">
                <input type="date" value={eventForm.date} onChange={e => setEventForm(p => ({ ...p, date: e.target.value }))} style={inputStyle} />
              </Field>
              <Field label="SEVERITY (1–5)">
                <input type="number" min={1} max={5} value={eventForm.severity} onChange={e => setEventForm(p => ({ ...p, severity: Number(e.target.value) }))} style={inputStyle} />
              </Field>
              <Field label="PRESIDENT">
                <input value={eventForm.president} onChange={e => setEventForm(p => ({ ...p, president: e.target.value }))} style={inputStyle} />
              </Field>
            </div>
            <Field label="SOURCE URL">
              <input value={eventForm.source_url} onChange={e => setEventForm(p => ({ ...p, source_url: e.target.value }))} style={inputStyle} placeholder="https://..." />
            </Field>
            <Field label="SOURCE LABEL">
              <input value={eventForm.source_label} onChange={e => setEventForm(p => ({ ...p, source_label: e.target.value }))} style={inputStyle} placeholder="e.g. Reuters" />
            </Field>
            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
              <button onClick={saveEvent} style={{
                flex: 1, fontFamily: 'var(--font-display)', fontWeight: 700,
                fontSize: '0.9rem', letterSpacing: '0.1em', padding: '0.65rem',
                borderRadius: 6, border: '1px solid var(--neon-green)',
                background: 'var(--neon-green-dim)', color: 'var(--neon-green)', cursor: 'pointer',
              }}>
                {editingEvent ? 'UPDATE EVENT' : 'CREATE EVENT'}
              </button>
              {editingEvent && (
                <button onClick={() => { setEditingEvent(null); setEventForm(BLANK_EVENT); }} style={{
                  fontFamily: 'var(--font-mono)', fontSize: '0.75rem', padding: '0.65rem 1rem',
          ...
          {/* Event list */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxHeight: '72vh', overflowY: 'auto' }}>
            {events.length === 0 && (
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: 'var(--text-dim)', padding: '2rem', textAlign: 'center', border: '1px solid var(--border)', borderRadius: 8 }}>
            ...
            {events.map(ev => (
              <div key={ev.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '1rem 1.25rem' }}>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--text-dim)', marginBottom: '0.75rem' }}>
                  {ev.category.toUpperCase()} · {ev.date} · severity {ev.severity}
                </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '2rem', alignItems: 'start' }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '1.5rem' }}>
            ...
            <Field label="PROMISE TEXT">
              <textarea value={promiseForm.text} onChange={e => setPromiseForm(p => ({ ...p, text: e.target.value }))} rows={3} style={{ ...inputStyle, resize: 'vertical' }} />
            </Field>
            <Field label="NOTES / CONTEXT">
              <textarea value={promiseForm.notes} onChange={e => setPromiseForm(p => ({ ...p, notes: e.target.value }))} rows={2} style={{ ...inputStyle, resize: 'vertical' }} />
            </Field>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              <Field label="CATEGORY">
              ...
              <Field label="DATE MADE">
                <input type="date" value={promiseForm.date_made} onChange={e => setPromiseForm(p => ({ ...p, date_made: e.target.value }))} style={inputStyle} />
              </Field>
              <Field label="PRESIDENT">
                <input value={promiseForm.president} onChange={e => setPromiseForm(p => ({ ...p, president: e.target.value }))} style={inputStyle} />
              </Field>
            </div>
            <Field label="SOURCE URL">
              <input value={promiseForm.source_url} onChange={e => setPromiseForm(p => ({ ...p, source_url: e.target.value }))} style={inputStyle} placeholder="https://..." />
            </Field>
            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxHeight: '72vh', overflowY: 'auto' }}>
            {promises.length === 0 && (
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: 'var(--text-dim)', padding: '2rem', textAlign: 'center', border: '1px solid var(--border)', borderRadius: 8 }}>
                NO PROMISES YET — CREATE ONE
              </div>
            )}
            {promises.map(p => {
              const colors = { kept: '#00ff87', broken: '#ff3b5c', reversed: '#b44dff', pending: '#ffb800' };
              return (
                <div key={p.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderLeft: '3px solid ' + (colors[p.status] || '#888'), borderRadius: 8, padding: '1rem 1.25rem' }}>
                  <div style={{ fontSize: '0.9rem', marginBottom: '0.25rem', color: 'var(--text-primary)' }}>{p.text}</div>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--text-dim)', marginBottom: '0.75rem' }}>
                    {p.status.toUpperCase()} · {p.category} · {p.date_made}
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
